...
static lv_color_t disp_buf1[3200];
static lv_color_t disp_buf2[3200];
...
static void my_disp_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_p)
{
  if(area->x2 <0) return;
  if(area->y2 <0) return;
  if(area->x1 >319) return;
  if(area->y1 >239) return;

  for(int y=area->y1;y<=area->y2;y++)
  {
	  for(int x=area->x1;x<=area->x2;x++)
	    {
		  ili9325_drv.WritePixel(x, y, color_p->full);
	  	  color_p++;
	    }
  }
...
uint8_t Display_Init(void)
{
	...
	lv_disp_draw_buf_init(&buf, disp_buf1, disp_buf2, 3200);
